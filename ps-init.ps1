...
"@
new-ItemWithContent -Path ".\startup.ps1" -ItemType "File" -Content $content
...
"@
new-ItemWithContent -Path ".\complete.ps1" -ItemType "File" -Content $content
...
`$psInitInstalled = Get-InstalledScript -Name "ps-inits" -ErrorAction SilentlyContinue

if(-not `$psInitInstalled) {
    Write-host "Installing PS-INIT"
    Install-Script -Name ps-init
}
...
Write-Host "Starting Completion Script..."
. .\Complete.ps1
Write-Host "Completion Script Finished."
"@
...
| File | Description |
|---|---|
| Startup.ps1           | contains code that will be executen on start |
| app\main.ps1          | main loop and will be executed untill aborted |
| complete.ps1          | contains code that will be executed once on completion |
